...
const UserProfileCard: React.FC<{ userId: string }> = ({ userId }) => {
  ...
  if (loading) {
    return <div style={{ padding: '20px', textAlign: 'center', color: '#666' }}>Loading user profile...</div>;
  }

  if (!userData) {
    return <div style={{ padding: '20px', textAlign: 'center', color: 'red' }}>User not found.</div>;
  }
  ...
  return (
    <div style={cardContainerStyle}>
      ...
      <div style={{ marginBottom: '15px', borderBottom: '1px dotted #e9ecef', paddingBottom: '10px' }}>
        <p style={{ fontWeight: 'bold', color: '#34495e' }}>
          Name: <span style={{ color: '#555' }}>{userData.name}</span>
        </p>
        <p style={{ color: '#555' }}>
          Email: <a href={`mailto:${userData.email}`} style={{ color: '#007bff', textDecoration: 'none' }}>{userData.email}</a>
        </p>
      </div>

      <div style={{ marginBottom: '15px' }}>
        <p style={{ fontSize: '0.95em', color: '#666' }}>
          Role: <span style={{ fontWeight: '600', color: '#444' }}>{userData.role}</span>
        </p>
        <p style={{ fontSize: '0.95em', color: '#666' }}>
          Status: <span style={statusBadgeStyle}>{userData.status.toUpperCase()}</span>
        </p>
        <p style={{ fontSize: '0.95em', color: '#666' }}>
          Last Login: <span style={{ fontStyle: 'italic' }}>{userData.lastLogin}</span>
        </p>
      </div>
